...
from modules.users.role_service import apply_role_change
...
def get_all_users(
    ...
    """List users with an optional name search and role filter, one page at a time.

    Returns the page and the total number of matches, so the caller can render a
    page count. Paginated because the admin panel used to load every account at
    once, which stops being reasonable as soon as the demo seeder adds a few
    hundred.
    """
    query = db.query(UserModel)
    if roles:
        query = query.filter(UserModel.role.in_(roles))
    if search and search.strip():
        query = query.filter(UserModel.name.ilike(f"%{search.strip()}%"))

    total = query.count()
    rows = (
        query.order_by(UserModel.role.desc(), UserModel.name.asc())
        .offset(offset)
        .limit(limit)
        .all()
    )
...
def update_user_role_database(db: Session, user_id: int, role: UserRole) -> UserModel:
    ...
    if not user:
        raise http_error(404, "user_not_found")
    try:
        # In the same transaction as the role itself: either the account moves
        # and its old-role data goes with it, or nothing happens.
        apply_role_change(db, user, role)
        user.role = role
        db.commit()
        db.refresh(user)
        return user
